...
function PAF.CombineEnemyEstimatedOffensivePower(units)
	local TotalPower = 0
	
	for v, Unit in pairs(units) do
		local UnitPower = PAF.GetEstimatedEnemyOffensivePower(Unit)
		TotalPower = (TotalPower + UnitPower)
	end
...
function PAF.GetEstimatedEnemyOffensivePower(hEnemy)
	local EnemyRawPower = hEnemy:GetRawOffensivePower()
	local EnemyMana = hEnemy:GetMana()
	local EnemyMaxMana = hEnemy:GetMaxMana()
	
	local EnemyPhysicalDmgOfPower = PAF.GetAttackDPS(hEnemy)
	
	local RemainingPower = (EnemyRawPower - EnemyPhysicalDmgOfPower)
	local EnemySpellDmgOfPower = (1.0 - RemapValClamped(EnemyMana, 0, EnemyMaxMana, 0, RemainingPower))
	
	local EstimatedPower = (EnemyPhysicalDmgOfPower + EnemySpellDmgOfPower)
...
function PAF.IsHealthRestoring(bot)
...
function PAF.IsManaRestoring(bot)
	return bot:HasModifier('modifier_clarity_potion') 
		or bot:HasModifier('modifier_fountain_aura') 
end

function PAF.IsItemAvailable(sItemName)
	local bot = GetBot()
...
function PAF.SwitchTreadsToInt(bot)
	bot:Action_ClearActions(false)
	
	local Treads = PAF.IsItemAvailable("item_power_treads")
	
	if Treads ~= nil and Treads:IsFullyCastable() then
		local TreadStat = Treads:GetPowerTreadsStat()
		
		if TreadStat == 2 then
			bot:ActionQueue_UseAbility(Treads)
			bot:ActionQueue_UseAbility(Treads)
			return
		elseif TreadStat == 0 then
			bot:ActionQueue_UseAbility(Treads)
			return
		end
	end
...
function PAF.SwitchTreadsToStr(bot)
	bot:Action_ClearActions(false)
...
function PAF.SwitchTreadsToAgi(bot)
	bot:Action_ClearActions(false)
...
function PAF.GetAttackCombatEfficiency(hAttacker, hReceiver)
	-- WIP
	
	if hAttacker:IsHero() then
		if hReceiver:IsCreep() then
			local ReceiverCreepType = hReceiver:GetAbilityInSlot(0)
		
			if ReceiverCreepType:GetName() == "creep_siege" then
				return 0.5
			end
		end
	end
	
	if hAttacker:IsCreep() then
		local AttackerCreepType = hAttacker:GetAbilityInSlot(0)
		
		if hReceiver:IsCreep() then
			local ReceiverCreepType = hReceiver:GetAbilityInSlot(0)
			
			if AttackerCreepType:GetName() == "creep_piercing" then
				if ReceiverCreepType:GetName() ~= "creep_siege" then
					return 1.5
				else
					return 0.7
...
function PAF.ShouldCastAbilityToFarm(hUnit, nManaCost, nManaThreshold, bPercentage)
	local nMana = hUnit:GetMana()
	
	if bPercentage then
		local nMaxMana = hUnit:GetMaxMana()
		
		if (nMana - nManaCost) >= (nMaxMana * nManaThreshold) then
			return true
		end
	else
		if (nMana - nManaCost) >= nManaThreshold then
			return true
		end
	end
...
function PAF.FilterExceptedUnit(tUnits, hExceptedUnit)
	local FilteredUnits = {}
	
	for x, Unit in pairs(tUnits) do
		if Unit ~= hExceptedUnit then
			table.insert(FilteredUnits, Unit)
		end
	end
...
function PAF.IsInCreepAttackingMode(hUnit)
	if hUnit:GetActiveMode() == BOT_MODE_PUSH_TOWER_TOP
	or hUnit:GetActiveMode() == BOT_MODE_PUSH_TOWER_MID
	or hUnit:GetActiveMode() == BOT_MODE_PUSH_TOWER_BOT
	or hUnit:GetActiveMode() == BOT_MODE_FARM then
		return true
	end
...
function PAF.CanDamageKillEnemy(hUnit, nDamage, nDamageType)
	if (nDamageType == DAMAGE_TYPE_MAGICAL or nDamageType == DAMAGE_TYPE_PURE)
	and PAF.IsMagicImmune(hUnit) then
		return false
	end
	
	local Damage = hUnit:GetActualIncomingDamage(nDamage, nDamageType)
	
	if Damage >= hUnit:GetHealth() then
		return true
	end
...
function PAF.IsHardBot(hUnit)
	...
	if hUnit:IsBot() then
		local nDifficulty = hUnit:GetDifficulty()
		
		if nDifficulty == 3 or nDifficulty == 4 then
			return true
		end
	end
...
function PAF.GetAbilityUsageDelay(hUnit)
	if hUnit:IsBot() then
		local nDifficulty = hUnit:GetDifficulty()
		
		if nDifficulty == 9 then
			return 3.0
		elseif nDifficulty == 1 then
			return 2.0
