...
function CDOTA_Bot_Script:GetAssignedLane()
    if self ~= nil
        ...
        -- Ensure consistency with original assignment if available
        if self.originalAssignedLane ~= nil then
            -- Convert lane number back to lane constant
            if self.lane == 1 then
                return LANE_TOP
            elseif self.lane == 2 then
                return LANE_MID
            elseif self.lane == 3 then
                return LANE_BOT
            end
        end
        return self.lane
