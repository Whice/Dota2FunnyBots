...
function X.Think(ownerBot, hMinionUnit)
    ...
    do
        ...
        then
            ...
            then
                hMinionUnit.cast_desire, hMinionUnit.cast_target = X.ConsiderUnitTarget(hMinionUnit,
                    hMinionUnit.abilities[i])
            ...
            if spellFunction then
                hMinionUnit.cast_desire, hMinionUnit.cast_target, hMinionUnit.cast_type = spellFunction(hMinionUnit,
                    hMinionUnit.abilities[i])
                if hMinionUnit.cast_desire > 0
                then
    ...
    then
        ...
        then
            ...
            do
                ...
                then
                    local nCreepCountAround = J.GetNearbyAroundLocationUnitCount(true, false, nJumpDist,
                        creep:GetLocation())
...
X.ConsiderSpellUsage['satyr_hellcaller_shockwave'] = function(hMinionUnit, ability)
    ...
    if J.IsGoingOnSomeone(bot) and J.IsInRange(bot, hMinionUnit, 1300) then
        local nLocationAoE = hMinionUnit:FindAoELocation(true, true, hMinionUnit:GetLocation(), nCastRange, 200,
            nCastPoint, 0)
