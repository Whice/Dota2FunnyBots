...
function J.PrintInitMessage(sFlag, sMessage)
    ...
    print('A Beginner AI ' ..
    string.sub(botName, 1, 4) .. ': ' .. string.sub(sFlag, 1, 5) .. ' with ' .. sMessage .. ' init successful!')
end
...
function J.CanNotUseAction(bot)
    return not bot:IsAlive()
        ...
        or
        (bot:IsInvulnerable() and not bot:HasModifier('modifier_fountain_invulnerability') and not bot:HasModifier('modifier_dazzle_nothl_projection_soul_debuff'))
    ...
    do
        ...
        then
            ...
            then
                ...
                nTotalDamage = nTotalDamage +
                ally:GetEstimatedDamageToTarget(true, target, allyDamageTime, DAMAGE_TYPE_PHYSICAL)
            end
        end
    end
...
function J.CanKillTarget(npcTarget, dmg, dmgType)
    return J.IsValid(npcTarget) and J.CanBeAttacked(npcTarget) and
    npcTarget:GetActualIncomingDamage(dmg, dmgType) >= npcTarget:GetHealth()
end
...
function J.IsGoingOnSomeone(bot)
    ...
    local bValidTarget = J.IsValidHero(botTarget) and not J.IsSuspiciousIllusion(botTarget) and
    J.IsInRange(bot, botTarget, 1600) and bot:GetTeam() ~= botTarget:GetTeam()
...
function J.GetAttackProDelayTime(bot, nCreep)
    ...
    then
        ...
        then
            nAttackDamageDelayTime = nAttackDamageDelayTime +
            (nDist - botAttackRange - ignoreDist / 1.2) / botMoveSpeed
        end
    end
...
function J.GetTotalAttackWillRealDamage(nUnit, nTime)
    return J.GetCreepAttackProjectileWillRealDamage(nUnit, nTime) +
...
local function GetHealthMultiplier(hUnit)
    ...
    if sUnitName == 'npc_dota_hero_huskar' then
        botHP = ((GetEffectiveHealthFromArmor(hUnit:GetHealth(), hUnit:GetArmor())) / hUnit:GetMaxHealth()) +
        (hUnit:GetHealthRegen() * 5.0 / hUnit:GetMaxHealth())
    ...
    elseif sUnitName == 'npc_dota_hero_medusa' then
        ...
        local nHealth = RemapValClamped(unitHealth / unitMaxHealth, 0, 1, 0, 1) * 0.2 +
        RemapValClamped(botMP, 0, 0.75, 0, 1) * 0.8
        ...
        then
            ...
            if GetTeam() == unit:GetTeam() then
                ...
                then
                    ...
                    if nDamage then
                        ourPower = ourPower +
                        0.5 * (bot:GetActualIncomingDamage(nDamage, DAMAGE_TYPE_PHYSICAL) - botHealthRegen) *
                        fMul_Illusion
                        ourPowerRaw = ourPowerRaw +
                        0.5 * (bot:GetActualIncomingDamage(nDamage, DAMAGE_TYPE_PHYSICAL) - botHealthRegen) *
                        fMul_Illusion
                    end
                else
                    ...
                    ourPower = ourPower +
                    (math.log(1 + unit:GetOffensivePower())) * (unit:GetAttackDamage() * unit:GetAttackSpeed()) * fMul
                    ourPowerRaw = ourPowerRaw +
                    (math.log(1 + unit:GetRawOffensivePower())) * (unit:GetAttackDamage() * unit:GetAttackSpeed()) * fMul
                end
            else
                if not unit:HasModifier('modifier_arc_warden_tempest_double')
                    and J.IsSuspiciousIllusion(unit)
                ...
                if bWithTeam then
                    if nDamage then
                        if string.find(sUnitName, 'warlock_golem') then
                            ...
                            ourPower = ourPower +
                            0.1 * (bot:GetActualIncomingDamage(nDamage, DAMAGE_TYPE_PHYSICAL) - botHealthRegen) *
                            fMul_Illusion
                            ourPowerRaw = ourPowerRaw +
                            0.1 * (bot:GetActualIncomingDamage(nDamage, DAMAGE_TYPE_PHYSICAL) - botHealthRegen) *
                            fMul_Illusion
