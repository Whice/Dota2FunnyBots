        ...
        if J.IsInLaningPhase() then
            ...
            for _, creep in pairs(tEnemyLaneCreeps) do
                if J.IsValid(creep) and J.IsInRange(bot, creep, 600) and creep:GetAttackTarget() == bot then
                    ...
                    creepCountAttackingDamage = creepCountAttackingDamage +
                    (bot:GetActualIncomingDamage(creep:GetAttackDamage() * creep:GetAttackSpeed() * 5.0, DAMAGE_TYPE_PHYSICAL) - bot:GetHealthRegen() * 5.0)
                end
            end
        ...
        for _, enemyHero in ipairs(unitList_enemies) do
            ...
            then
                ...
                for _, allyHero in ipairs(unitList_allies) do
                    ...
                    then
                        fAllyDamage = fAllyDamage +
                        allyHero:GetEstimatedDamageToTarget(true, enemyHero, 3.0, DAMAGE_TYPE_ALL) -
                        enemyHero:GetHealthRegen() * 3.0
                ...
                for _, possibleEnemyHero in ipairs(unitList_enemies) do
                    ...
                    then
                        fEnemyDamage = fEnemyDamage +
                        possibleEnemyHero:GetEstimatedDamageToTarget(false, bot, 3.0, DAMAGE_TYPE_ALL)
                    end
                end
            ...
            for _, enemy in pairs(nEnemyHeroes) do
                ...
                then
                    if J.IsInRange(bot, enemy, enemy:GetAttackRange() + 150) then
                        bot:Action_MoveToLocation(J.VectorAway(botLocation, enemy:GetLocation(),
                            enemy:GetAttackRange() * 2))
                        return
                    end
                end
            end
            ...
            if J.IsValidBuilding(nEnemyTowers[1]) and nEnemyTowers[1]:GetAttackTarget() == bot and not bTeamFight then
                ...
                for _, unit in pairs(unitList) do
                    if J.IsValid(unit) and unit:GetAttackTarget() == bot then
                        ...
                        bot:GetActualIncomingDamage(unit:GetAttackDamage() * unit:GetAttackSpeed() * 3.0,
                ...
                if dist < botAttackRange then
                    if not J.CanBeAttacked(__target) --[[or (bot:GetLastAttackTime() + bot:GetSecondsPerAttack()) > GameTime()]] then
                        if dist < botAttackRange - 100 then
                            bot:Action_MoveToLocation(J.VectorAway(botLocation, __target:GetLocation(),
                                botAttackRange - dist - 100))
                            return
                        elseif dist > botAttackRange - 100 then
                            bot:Action_MoveToLocation(J.VectorTowards(botLocation, __target:GetLocation(),
                                dist - botAttackRange - 100))
                            return
                        end
                    else
                        -- if dist < botAttackRange then
...
function X.GetWeakestNearbyHero(bEnemy, nRadius)
    ...
    for _, hero in ipairs(nNearbyHeroes) do
        ...
        then
            local heroScore = hero:GetActualIncomingDamage(bot:GetAttackDamage() * bot:GetAttackSpeed() * 3.0,
                DAMAGE_TYPE_PHYSICAL) / (hero:GetHealth() + hero:GetHealthRegen() * 3.0)
