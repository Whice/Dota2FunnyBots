...
function AbilityUsageThink()
	...
	if JetpackDesire > 0 then
		...
		bot:ActionQueue_UseAbility(Jetpack)
		return
	end
	...
	if OverclockingDesire > 0 then
		PAF.SwitchTreadsToInt(bot)
		bot:ActionQueue_UseAbility(Overclocking)
		return
	end
	...
	if PowerCogsDesire > 0 then
		PAF.SwitchTreadsToInt(bot)
		bot:ActionQueue_UseAbility(PowerCogs)
		return
	end
	...
	if BatteryAssaultDesire > 0 then
		PAF.SwitchTreadsToInt(bot)
		bot:ActionQueue_UseAbility(BatteryAssault)
		return
	end
	...
	if HookshotDesire > 0 then
		PAF.SwitchTreadsToInt(bot)
		bot:ActionQueue_UseAbilityOnLocation(Hookshot, HookshotTarget)
		return
	end
	...
	if RocketFlareDesire > 0 then
		PAF.SwitchTreadsToInt(bot)
		bot:ActionQueue_UseAbilityOnLocation(RocketFlare, RocketFlareTarget)
		return
	end
end
