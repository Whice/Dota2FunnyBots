    ...
    if J.IsFarming(bot) and fManaAfter > (300 / bot:GetMaxMana()) then
        if bAttacking then
            if J.IsValid(nEnemyCreeps[1])
                ...
                and bAttacking
            then
                local nLocationAoE = bot:FindAoELocation(true, false, nEnemyCreeps[1]:GetLocation(), 0, nRadius,
                    nCastPoint, 0)
...
function X.ConsiderBattleTrance()
    ...
    if J.IsGoingOnSomeone(bot) then
        ...
        then
            ...
            local totalAttackDamge = nEnemyHeroes[1]:GetActualIncomingDamage(
                bot:GetAttackDamage() * (bot:GetAttackSpeed() + nBonusAS / 100) * fDuration, DAMAGE_TYPE_PHYSICAL)
