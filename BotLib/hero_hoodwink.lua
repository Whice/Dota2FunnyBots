    ...
    local HeroBuild = {
        ['pos_1'] = {
            [1] = {
                ...
                ['buy_list'] = {
                    "itemx_ranged_magic_wand_wraith_band_power_treads",
        ...
        ['pos_2'] = {
            [1] = {
                ...
                ['buy_list'] = {
                    "itemx_ranged_bottle_wraith_band_power_treads",
                ...
                ['sell_list'] = {
                    ...
                    "item_falcon_blade", "item_butterfly",
            ...
            if J.CanKillTarget(enemyHero, damage, DAMAGE_TYPE_PHYSICAL) then
                ...
                then
                    return BOT_ACTION_DESIRE_HIGH,
                        J.GetXUnitsTowardsLocation2(bot:GetLocation(), J.GetCorrectLoc(enemyHero, nDelay), nCastRange),
                        'loc'
                end
            end
        end
        ...
        then
            ...
            for _, enemy in pairs(nAllyInRangeEnemy) do
                ...
                then
                    ...
                    then
                        local target_loc = J.GetXUnitsTowardsLocation2(bot:GetLocation(), J.GetCorrectLoc(enemy, nDelay),
                            nCastRange)
        ...
        then
            ...
            then
                if #nTrees > 0 then
                    ...
                    local target_loc = J.GetXUnitsTowardsLocation2(bot:GetLocation(), J.GetCorrectLoc(botTarget, nDelay),
                        nCastRange)
                    local nLocationAoE = bot:FindAoELocation(true, true, target_loc, 0, nRadius, 0, 0)
        ...
        then
            ...
            then
                ...
                if J.CanKillTarget(enemyHero, nDamage, DAMAGE_TYPE_MAGICAL) then
                    local nLocationAoE = bot:FindAoELocation(true, true, J.GetCorrectLoc(enemyHero, nDelay), 0, nRadius,
                        0, 0)
                    if nLocationAoE.count > 0 then
                        if #nTrees > 0 then
                            return BOT_ACTION_DESIRE_HIGH, nLocationAoE.targetloc, false
                ...
                if bCastCombo then
                    local nBounceCount = AcornShot:GetSpecialValueInt('bounce_count')
                    local nDamage_acorn = AcornShot:GetSpecialValueInt('acorn_shot_damage') *
                        (bot:GetAttackDamage() * 0.75)
                    local totalDamage = enemyHero:GetActualIncomingDamage(nDamage, DAMAGE_TYPE_MAGICAL)
                    totalDamage = totalDamage +
                        enemyHero:GetActualIncomingDamage(nDamage_acorn * nBounceCount, DAMAGE_TYPE_PHYSICAL)
                    if totalDamage >= enemyHero:GetHealth() then
                        local nLocationAoE = bot:FindAoELocation(true, true, J.GetCorrectLoc(enemyHero, nDelay), 0,
                            nRadius, 0, 0)
                        if nLocationAoE.count > 0 then
                            if #nTrees > 0 then
                                return BOT_ACTION_DESIRE_HIGH, nLocationAoE.targetloc, false
        ...
        then
            ...
            for _, enemy in pairs(nAllyInRangeEnemy) do
                ...
                then
                    ...
                    if #nTrees > 0 then
                        local nDelay = (GetUnitToUnitDistance(bot, enemy) / nSpeed) + nCastPoint
                        local nLocationAoE = bot:FindAoELocation(true, true, J.GetCorrectLoc(enemy, nDelay), 0, nRadius,
                            0, 0)
                        if nLocationAoE.count > 0 then
                            return BOT_ACTION_DESIRE_HIGH, nLocationAoE.targetloc, false
                        end
...
function X.ConsiderSharpshooterRelease()
    ...
    local nDamage = RemapValClamped(DotaTime(), SharpshooterStartTime, SharpshooterStartTime + nMaxChargeTime, nMinDamage,
        nMaxDamage)
