    ...
    if J.IsGoingOnSomeone(bot) then
        ...
        then
            ...
            if #nInRangeAlly >= #nInRangeEnemy and #nInRangeAlly2 >= #nInRangeEnemy2 and bStronger then
                local vLocation = J.GetCorrectLoc(botTarget,
                    (GetUnitToUnitDistance(bot, botTarget) / nSpeed) + nCastPoint)
...
function X.SetRatios()
    ...
    -- Stats
    count = 0
    if bot:GetLevel() >= 26 then
        count = 7
    elseif bot:GetLevel() >= 24 then
        count = 6
    elseif bot:GetLevel() >= 23 then
        count = 5
    elseif bot:GetLevel() >= 22 then
        count = 4
    elseif bot:GetLevel() >= 21 then
        count = 3
    elseif bot:GetLevel() >= 19 then
        count = 2
    elseif bot:GetLevel() >= 17 then
        count = 1
    end
