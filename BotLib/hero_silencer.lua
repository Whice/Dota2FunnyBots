...
then
    ...
    local HeroBuild = {
        ['pos_1'] = {
            [1] = {
                ...
                ['buy_list'] = {
                    "itemx_ranged_magic_wand_null_talisman_power_treads",
                ...
                ['sell_list'] = {
                    ...
                    "item_null_talisman", "item_yasha_and_kaya",
        ...
        ['pos_2'] = {
            [1] = {
                ...
                ['buy_list'] = {
                    "itemx_ranged_bottle_null_talisman_power_treads",
                ...
                ['sell_list'] = {
                    ...
                    "item_falcon_blade", "item_octarine_core",
                    "item_power_treads", "item_travel_boots",
                },
            },
        },
        ...
        ['pos_4'] = {
            [1] = {
                ...
                ['buy_list'] = {
                    "item_soft_support_ranged_outfit",
        ...
        ['pos_5'] = {
            [1] = {
                ...
                ['buy_list'] = {
                    "item_hard_support_ranged_outfit",
    ...
    nAbilityBuildList, nTalentBuildList, X['sBuyList'], X['sSellList'] = J.SetUserHeroInit(nAbilityBuildList,
        nTalentBuildList, X['sBuyList'], X['sSellList'])
...
function X.ConsiderQ()
    ...
    local nEnemysWeakestLaneCreepsInRange, nEnemysWeakestLaneCreepsInRangeHealth = X.sil_GetWeakestUnit(
        nEnemysLaneCreepsInRange)
    local nEnemysWeakestLaneCreepsInBonus, nEnemysWeakestLaneCreepsInBonusHealth = X.sil_GetWeakestUnit(
        nEnemysLaneCreepsInBonus)

    local nTowers = bot:GetNearbyTowers(1000, true)

    local nCanKillHeroLocationAoE = bot:FindAoELocation(true, true, bot:GetLocation(), nCastRange, nRadius, 0,
        0.7 * nDamage)
    ...
    then
        local nCanHurtHeroLocationAoENearby = bot:FindAoELocation(true, true, bot:GetLocation(), nCastRange - 300,
            nRadius, 0.8, 0)
...
function X.ConsiderW()
    ...
    local nAbilityDamage = abilityW:GetSpecialValueInt("intellect_damage_pct") / 100 *
        bot:GetAttributeValue(ATTRIBUTE_INTELLECT) * (1 + bot:GetSpellAmp())
...
function X.ConsiderE()
    ...
    for _, enemyHero in pairs(nEnemyHeroes) do
        ...
        then
            local nDamage = nBaseDamage +
                math.abs(bot:GetAttributeValue(ATTRIBUTE_INTELLECT) - enemyHero:GetAttributeValue(ATTRIBUTE_INTELLECT))
    ...
    if J.IsInTeamFight(bot, 1200) then
        ...
        if npcMostDangerousEnemy ~= nil then
            if bAoETalent then
                local nLocationAoE = bot:FindAoELocation(true, true, npcMostDangerousEnemy:GetLocation(), 0, nRadius, 0,
                    0)
                if nLocationAoE.count >= 2 then
                    return BOT_ACTION_DESIRE_HIGH, nLocationAoE.targetloc, true
                else
...
function X.sil_RealDamage(nAttackDamage, nAbilityDamage, unit)
    local RealDamage = unit:GetActualIncomingDamage(nAttackDamage, DAMAGE_TYPE_PHYSICAL) +
        unit:GetActualIncomingDamage(nAbilityDamage, DAMAGE_TYPE_PURE)
