...
function X.ConsiderDecay()
    ...
    if J.IsLaning(bot) and J.IsInLaningPhase() and fManaAfter > fManaThreshold and (J.IsCore(bot) or not J.IsThereCoreNearby(800)) then
        if J.IsValid(nEnemyCreeps[1])
            ...
            and not J.IsRunning(nEnemyCreeps[1])
        then
            local nLocationAoE = bot:FindAoELocation(true, false, nEnemyCreeps[1]:GetLocation(), 0, nRadius, 0,
                nDamage * 2)
...
function X.GetAllHeroCreepNearbyCount(vLocation, nRadius)
    ...
    return nLocationAoE_allyHeroes.count + nLocationAoE_allyCreeps.count + nLocationAoE_enemyHeroes.count +
        nLocationAoE_enemyCreeps.count
end
