            ...
            if not J.IsInRange(bot, botTarget, nCastRange) then
                if J.IsChasingTarget(bot, botTarget) then
                    return BOT_ACTION_DESIRE_HIGH,
                        J.VectorTowards(bot:GetLocation(), botTarget:GetLocation(), nCastRange)
                end
            else
            ...
            then
                if J.IsInRange(bot, enemy, nCastRange) then
                    ...
                    if J.IsChasingTarget(enemy, bot) and botHP < 0.5 and bot:WasRecentlyDamagedByAnyHero(3.0) then
                        return BOT_ACTION_DESIRE_HIGH,
                            J.VectorTowards(bot:GetLocation(), enemy:GetLocation(), nCastRange)
                    end
                end
            end
        end
...
function X.ConsiderTreeVolley()
    ...
    if J.IsInTeamFight(bot, 1200) then
        ...
        if #nTrees >= 3 and #nInRangeAlly >= #nInRangeEnemy then
            local nLocationAoE = bot:FindAoELocation(true, true, bot:GetLocation(), nCastRange, nSplashRadius, fCastPoint,
                0)
            ...
            then
                ...
                if #nInRangeAlly >= #nInRangeEnemy or bWeAreStronger then
                    ...
                    if allyTarget ~= nil then
                        local nLocationAoE__AllyHeroes = bot:FindAoELocation(false, true, allyTarget:GetLocation(), 0,
                            275, 0, 0)
                        local nLocationAoE__EnemyHeroes = bot:FindAoELocation(true, true, allyTarget:GetLocation(), 0,
                            275, 0, 0)
                        local nLocationAoE__AllyCreeps = bot:FindAoELocation(false, false, allyTarget:GetLocation(), 0,
                            275, 0, 0)
                        local nLocationAoE__EnemyCreeps = bot:FindAoELocation(true, false, allyTarget:GetLocation(), 0,
                            275, 0, 0)
