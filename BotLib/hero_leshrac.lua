...
function X.ConsiderSplitEarth()
        ...
        then
            ...
            then
                ...
                if not J.IsInRange(bot, enemyHero, nCastRange) and J.IsInRange(bot, enemyHero, nCastRange + nRadius) then
                    nLocationAoE = bot:FindAoELocation(true, true,
                        J.Site.GetXUnitsTowardsLocation(bot, vLocation, nCastRange), 0, nRadius, 0, 0)
                    return BOT_ACTION_DESIRE_HIGH, nLocationAoE.targetloc
                else
    ...
    if J.IsGoingOnSomeone(bot) then
        ...
        then
            nLocationAoE = bot:FindAoELocation(true, true, J.GetCorrectLoc(botTarget, nDelay + nCastPoint), 0, nRadius, 0,
                0)
            return BOT_ACTION_DESIRE_HIGH, nLocationAoE.targetloc
        end
    end
    ...
    then
        for _, enemyHero in pairs(nEnemyHeroes) do
            ...
            then
                nLocationAoE = bot:FindAoELocation(true, true, J.GetCorrectLoc(enemyHero, nDelay + nCastPoint), 0,
                    nRadius, 0, 0)
                return BOT_ACTION_DESIRE_HIGH, nLocationAoE.targetloc
            end
        end
