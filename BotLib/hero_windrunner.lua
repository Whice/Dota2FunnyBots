...
function X.GetShackleHeroTarget(hSource, hTarget, nRadius, nMaxAngle)
    ...
    do
        ...
        then
            local angle = X.GetAngleWithThreeVectors(hSource:GetLocation(), enemyHero:GetLocation(),
                hTarget:GetLocation())
...
function X.CanShackleToHero(hSource, hTarget, nRadius, nMaxAngle)
    ...
    do
        if J.IsValidHero(enemyHero)
            and enemyHero ~= hTarget
        then
            local angle = X.GetAngleWithThreeVectors(hSource:GetLocation(), hTarget:GetLocation(),
                enemyHero:GetLocation())
            if angle <= nMaxAngle then
                return true
            end
        end
