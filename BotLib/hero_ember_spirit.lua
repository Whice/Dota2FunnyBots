...
function X.ConsiderFireRemnant()
    ...
    local nSpeed = bot:GetCurrentMovementSpeed() *
        (FireRemnant:GetSpecialValueInt('speed_multiplier') / (bot:HasScepter() and 50 or 100))
    local botTarget = J.GetProperTarget(bot)
    ...
    then
        ...
        then
            return BOT_ACTION_DESIRE_HIGH,
                J.Site.GetXUnitsTowardsLocation(bot, J.GetTeamFountain(), RandomInt(600, 1400))
        end
    end
