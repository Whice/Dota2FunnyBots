...
local function AbilityLevelUpComplement()
    ...
    then
        ...
        -- Store original lane and role assignment to prevent changes during game
        if bot.originalAssignedLane == nil then
            bot.originalAssignedLane = bot:GetAssignedLane()
            bot.originalRole = bot.theRole
            bot.originalPosition = J.GetPosition(bot)
        end
    end
    
    -- Ensure role consistency throughout the game
    if bot.originalRole ~= nil and bot.theRole ~= bot.originalRole then
        bot.theRole = bot.originalRole
    end
    ...
    then
        ...
        then
            ...
            then
                ...
                then
                    chatString = J.Chat.GetStopReplyString()
                end
...
X.ConsiderItemDesire["item_tpscroll"] = function(hItem)
    ...
    -- if nMode == BOT_MODE_LANING and J.IsEarlyGame() and DotaTime() > 120
    -- then
