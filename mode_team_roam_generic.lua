...
function GetDesire()
	...
	-- fix lanes for these two - maintain their originally assigned lanes
	if botName == 'npc_dota_hero_elder_titan'
	or botName == 'npc_dota_hero_wisp'
	then
		if bot.lane == nil then
			-- Store original lane assignment to maintain role consistency
			if bot.originalLane == nil then
				-- Capture the original lane on first run to prevent role changes
				local botPos = J.GetPosition(bot)
				if botPos == 1 or botPos == 5 then
					if GetTeam() == TEAM_RADIANT then
						bot.originalLane = 3  -- Safe lane (bot for Radiant)
					else
						bot.originalLane = 1  -- Safe lane (top for Dire)
					end
				elseif botPos == 2 then
					bot.originalLane = 2  -- Mid lane
				elseif botPos == 3 or botPos == 4 then
					if GetTeam() == TEAM_RADIANT then
						bot.originalLane = 1  -- Off lane (top for Radiant)
					else
						bot.originalLane = 3  -- Off lane (bot for Dire)
					end
				end
			end
			-- Always use the originally assigned lane
			bot.lane = bot.originalLane
		end
	end
